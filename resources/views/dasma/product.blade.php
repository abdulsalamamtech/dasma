            ...
            <div class="container">
                ...
                <div class="-mx-5 flex flex-col justify-between pt-16 pb-24 lg:flex-row">
                    <div class="flex flex-col-reverse justify-between px-5 sm:flex-row-reverse lg:w-1/2 lg:flex-row"
                        x-data="{ selectedImage: '{{ $product->banner->url }}' }">
                            ...
                            <div class="relative mr-3 w-28 pb-5 sm:w-32 sm:pr-0 lg:w-24 xl:w-28">
                                <div
                                    ...
                                    <img class="cursor-pointer object-cover" @click="selectedImage = $event.target.src"
                                        alt="product image {{ $product->name }}" src="{{ $product->banner->url }}">
                                </div>
                            </div>
                            ...
                            @forelse ($product->variations as $pro_variation)
                                ...
                                <div class="relative mr-3 w-28 pb-5 sm:w-32 sm:pr-0 lg:w-24 xl:w-28">
                                    <div
                                        class="relative flex w-full items-center justify-center rounded border border-grey bg-v-pink">
                                        <img class="cursor-pointer object-cover" @click="selectedImage = $event.target.src"
                                            alt="product image {{ $pro_variation->title }}"
                                            src="{{ $pro_variation->asset->url }}">
                                    </div>
                                </div>
                            @empty
                                <div class="relative mr-3 w-28 pb-5 sm:w-32 sm:pr-0 lg:w-24 xl:w-28">
                    ...
                    <div class="px-5 pt-8 lg:w-1/2 lg:pt-0">
                        <div class="mb-8 border-b border-grey-dark">
                            <div class="flex items-center">
                                ...
                                <p
                                    class="ml-8 rounded-full bg-primary px-5 py-2 font-hk text-sm font-bold uppercase leading-none text-white">
                                    {{-- 20% off --}}
                            ...
                            <div class="w-2/3 sm:w-5/6 grid grid-cols-6 sm:grid-cols-6 md:grid-cols-6 gap-2">
                                @forelse (array_unique($colors) as $color)
                                    ...
                                    <label class="cursor-pointer">
                                        <input id="productColor-{{ $loop->index }}" type="radio" name="color"
                                            value="{{ $color }}" class="peer hidden"
                                            onchange="updateHiddenInput(this)" />
                                        <div
                                            class="w-8 h-8 rounded-full border-2 border-gray-300 bg-[{{ $color }}] peer-checked:ring-2 peer-checked:ring-[{{ $color }}] peer-checked:border-[{{ $color }}]">
                                        </div>
                                    </label>
                                @empty
                                    <label class="cursor-pointer">
                                        <input id="productColor-default" type="radio" name="color" value="#000000"
                                            class="peer hidden" onchange="updateHiddenInput(this)" />
                                        <div
                                            class="w-8 h-8 rounded-full border-2 border-gray-300 bg-black peer-checked:ring-2 peer-checked:ring-gray-700 peer-checked:border-gray-700">
                                        </div>
                                    </label>
                                @endforelse
                            </div>
                        ...
                        <div class="flex items-center justify-between pb-8">
                            ...
                            <div class="flex w-2/3 sm:w-5/6" x-data="{ productQuantity: 1 }">
                                <label for="quantity-form" class="relative block h-0 w-0 overflow-hidden">Quantity
                                    form</label>
                                <input name="quantity" type="number" id="quantity-form"
                                    value="{{ $product?->cartItem()?->quantity ?? 1 }}"
                                    class="quantity form-quantity form-input w-16 rounded-r-none py-0 px-2 text-center"
                                    x-model="productQuantity" min="1" max="{{ $product->stock }}">
                                <div class="flex flex-col">
                                    <span
                                        class="increment-cart flex-1 cursor-pointer rounded-tr border border-l-0 border-grey-darker bg-white px-1"
                                        @click="productQuantity++">
                                        <i class="bx bxs-up-arrow pointer-events-none text-xs text-primary"></i>
                                    </span>
                                    <span
                                        class="decrement-cart flex-1 cursor-pointer rounded-br border border-t-0 border-l-0 border-grey-darker bg-white px-1"
                        ...
                        <div id="productAction" class="group flex pb-8">
                            @if ($product?->cartItem())
                                ...
                                <div id="updateCartFromProduct"
                                    class="updateCartFromProduct btn btn-primary py-4 cursor-pointer">
                ...
                <div class="pb-16 sm:pb-20 md:pb-24" x-data="{ activeTab: 'description' }">
                    ...
                    <div class="tab-content relative">
                        ...
                        <div :class="{ 'active': activeTab=== 'additional-information' }"
                            class="tab-pane bg-grey-light py-10 transition-opacity md:py-16 active" role="tabpanel">
                                ...
                                <div class="splide__track">
                                    <div class="splide__list relative pt-12">
                                        ...
                                        @forelse ($related_products as $product)
                                            {{-- START: Product Card --}}
                                            <div
                                                class="splide__slide group relative pt-16 md:pt-0 border-l border-grey-dark last:border-0">
                                                <div
                                                    class="w-full mx-auto border-b border-transparent pb-8 text-center sm:text-left md:pl-4 text-center">
                                                    <div
                                                        class="flex items-center justify-center pt-3 sm:justify-start xl:pt-5">
                                                        <i class="bx bxs-star text-primary"></i>
                                                        <i class="bx bxs-star text-primary"></i>
                                                        <i class="bx bxs-star text-primary"></i>
                                                        <i class="bx bxs-star text-primary"></i>

                ...
                <div class="bg-grey-light py-8 px-0 md:px-8">
                    ...
                    <div class="hidden sm:block">
                        <div class="flex justify-between pb-3">
                            ...
                            <div class="w-1/4 text-center xl:w-1/5">
                                <span class="font-hkbold text-sm uppercase text-secondary">Reference</span>
                            </div>
                            <div class="w-1/4 text-center xl:w-1/5">
                                <span class="font-hkbold text-sm uppercase text-secondary">Items Qty</span>
                            </div>
                    ...
                    @forelse ($transactions as $transaction)
                        <div
                            ...
                            {{-- S/N --}}
                            <div class="w-full btransaction-b btransaction-grey-dark pb-4 text-center sm:w-1/5 sm:btransaction-b-0 sm:pb-0">
                                <span
                                    class="font-hkbold block pt-3 pb-2 text-center text-sm uppercase text-secondary sm:hidden">S/N</span>
                                <span class="font-hk text-secondary">TN{{ $transaction->id . $transaction->order_id }}</span>
                            </div>
                            {{-- Reference --}}
                            <div class="w-full btransaction-b btransaction-grey-dark pb-4 text-center sm:w-1/5 sm:btransaction-b-0 sm:pb-0">
                                <span
                                    class="font-hkbold block pt-3 pb-2 text-center text-sm uppercase text-secondary sm:hidden">Reference</span>
                                <span class="font-hk text-secondary">{{ $transaction->reference }}</span>
                            </div>
                            {{-- Quantity --}}
                            <div class="w-full btransaction-b btransaction-grey-dark pb-4 text-center sm:w-1/5 sm:btransaction-b-0 sm:pb-0">
                                <span
                                    class="font-hkbold block pt-3 pb-2 text-center text-sm uppercase text-secondary sm:hidden">Total Qty</span>
                                <span class="font-hk text-secondary">{{ $transaction->order->items->count() }}</span>
                            </div>
                            {{-- Total amount --}}
                            <div
                                class="w-full btransaction-b btransaction-grey-dark pb-4 text-center sm:w-1/6 sm:btransaction-b-0 sm:pr-6 sm:pb-0 sm:text-right xl:w-1/5 xl:pr-16">
                                <span
                                ...
                                <span class="font-hk text-secondary">
                                    ...
                                    {{ $transaction->order->total_amount ?? 0 }}
                                </span>
                            </div>
                            {{-- Discount --}}
                            <div
                            ...
                            <div
                                ...
                                <span class="font-hk text-secondary">
                                    {{ App\Helpers\Setup::currency('sign') }}
                                    {{ $transaction->order->shipping_fee ?? 0 }}
                                </span>
                            </div>
                            {{-- Total Payment --}}
                            <div
                            ...
                            <div class="w-full text-center sm:w-3/10 sm:text-right md:w-1/4 xl:w-1/5">
                                <div class="pt-3 sm:pt-0">
                                    ...
                                    <span
                                        class="bg-{{ $transaction->statusColor() }}-100 btransaction btransaction-{{ $transaction->statusColor() }}-400 px-4 py-3 inline-block rounded font-hk text-{{ $transaction->statusColor() }}-400">
                                        {{-- In Progress --}}
                                        {{ $transaction->status }}
                                    </span>
                                </div>
                            ...
                            <div class="w-full text-center sm:w-3/10 sm:text-right md:w-1/4 xl:w-1/5">
                                <div class="pt-3 sm:pt-0">
                                    ...
                                    <div>
                                        ...
                                        <!-- Dropdown menu -->
                                        <div id="dropdownDots{{ $transaction->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600 text-left font-hk">
                                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton{{ $transaction->id }}">
                                                @if ($transaction->status == 'pending' && $transaction->order->status == 'pending' && $transaction->order->paid == 'no')
                                                    <li>
                                                        <a href="{{ route('account.proceed-to-checkout', $transaction->order->id) }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Make Payment</a>
                                                    </li>
                                                @elseif($transaction->status !== 'pending' && $transaction?->status == 'successful')
                                                    <li>
                                                        <a href="{{ route('account.transactions.index') }}?reference={{ $transaction?->reference }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Reverify Payment</a>
                                                    </li>                                            
                                                @else
                                                    <li>
                                                        <a href="{{ route('account.transactions.index') }}?reference={{ $transaction?->reference }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Verify Payment</a>
                                                    </li>  
                                                @endif
